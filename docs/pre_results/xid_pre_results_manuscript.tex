...
\vspace{6pt}
{\small \textsc{Preprint} \(\cdot\) Version 0.2 \(\cdot\) 12 August 2026
...
scale-free
diagnostic measuring distance from the diagonal-plus-rank-$K$ set whose
rejection supports genuine cross-impact, the reverse of the usual reading.
We verify the derivation in a preregistered known-truth experiment with
$N=30$, $K=3$, $T=10^{7}$, and register, without reporting, a falsification
protocol for the empirical stage.
\end{abstract}
...
This pre-results preprint reports completed derivations and one preregistered
known-truth simulation. It does not report the registered premise test, a
market-data study, a training sample, a holdout, or an economic evaluation. No
registered stochastic stream has run and no external market data has been
accessed.
\end{minipage}

\vspace{6pt}
\noindent\textbf{Keywords:} cross-impact; order-flow imbalance; latent factors;
partial identification; low-rank structure; preregistration.

\vspace{4pt}
\noindent\textbf{JEL:} G14, C58, C18.
...
$\psi_3$ at exact structure & $1.76\times10^{-15}$ \\
